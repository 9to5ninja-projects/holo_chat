...
function activate(context) {
    ...
    const progressReporter = new progressReporter_1.ProgressReporter();
    const errorHandler = new errorHandler_1.ErrorHandler(outputChannel);
    ...
    const holoTreeProvider = new treeData_1.MemoryTreeProvider(bridge);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('holographicMemory.openGraph', () => {
        graphPanel_1.HoloGraphPanel.createOrShow(context.extensionUri, bridge);
        ...
        try {
            ...
            if (result.success) {
                ...
                graphPanel_1.HoloGraphPanel.currentPanel?.postMessage({
                    type: 'capsuleCreated',
                    data: result.capsule
                });
            }
        ...
        try {
            ...
            if (!graphPanel_1.HoloGraphPanel.currentPanel) {
        ...
        try {
            ...
            const result = await progressReporter.withDetailedProgress("Indexing workspace for holographic annotations", async (progress) => {
                progress.report({ increment: 10, message: "Initializing Python bridge..." });
                try {
                    await bridge.ensureRunning();
                }
                catch (err) {
                    await errorHandler.handlePythonBridgeError(err);
                    throw err;
                }
            ...
            await errorHandler.handleIndexingError(err);
        }
        ...
        try {
            ...
            if (annotations.length === 0) {
                await errorHandler.showInfo('No annotations found', 'Run "Index Workspace" first to scan for holographic memory annotations.');
                return;
            }
            ...
            const result = {
                ...
                files_scanned: 0,
                files_with_annotations: new Set(annotations.map((ann) => ann.file_path)).size,
                created_capsules: [],
                annotations_by_file: annotations.reduce((acc, ann) => {
...
`;
    ...
    vscode.commands.registerCommand('holographicMemory.goToDefinition', async (source) => {
        ...
        try {
            ...
            const range = new vscode.Range(new vscode.Position(lineStart, 0), new vscode.Position(lineEnd, 0));
            // Select the range and reveal it
            editor.selection = new vscode.Selection(range.start, range.end);
            editor.revealRange(range, vscode.TextEditorRevealType.InCenter);
            // Show a brief message
            vscode.window.setStatusBarMessage(`📍 Jumped to ${source.style} annotation at line ${source.lineStart}`, 3000);
        }
        catch (err) {
            await errorHandler.handleFileSystemError(err, source.file);
        }
    }), 
    // Explain Capsule with real Ollama integration
    vscode.commands.registerCommand('holographicMemory.explainCapsule', async (capsuleId) => {
        if (!capsuleId) {
        ...
        try {
            await bridge.ensureRunning();
            const result = await progressReporter.withDetailedProgress("Generating capsule explanation with Ollama", async (progress) => {
...
`;
                ...
                await errorHandler.showWarning('Failed to generate explanation', result.error || 'Unknown error occurred', [
    ...
    vscode.commands.registerCommand('holographicMemory.rebuildIndex', async () => {
        try {
            await bridge.ensureRunning();
            const result = await progressReporter.withDetailedProgress("Rebuilding FAISS index for fast retrieval", async (progress) => {
            ...
            if (result.ok) {
                vscode.window.showInformationMessage(`✅ FAISS index rebuilt successfully! Indexed ${result.size} capsules.`);
            }
            else {
                await errorHandler.showWarning('FAISS index rebuild failed', result.reason || 'Unknown error', [
    ...
    vscode.commands.registerCommand('holo.openGraph', () => {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(`Scanned ${res.files_scanned || 0} files • indexed ${res.total_annotations || 0} annotations (created ${res.created_capsules?.length || 0} capsules)`);
                if (res.errors?.length) {
                    const items = res.errors.slice(0, 5).map((e) => `${e.file}: ${e.error}`);
...
`;
    ...
    }), vscode.commands.registerCommand('holo.gotoCapsule', async (capsule) => {
        ...
        try {
            ...
            vscode.window.setStatusBarMessage(`📍 Jumped to ${capsule.source.style} annotation at line ${capsule.source.lineStart}`, 3000);
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to open source: ${error}`);
        }
    }), vscode.commands.registerCommand('holo.explainCapsule', async (capsule) => {
        try {
            await bridge.ensureRunning();
