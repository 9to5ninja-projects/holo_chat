...
class HolographicAnnotation:
    ...
    def __init__(self, capsule_id: str, file_path: Path, line_start: int, line_end: int):
        ...
        # Enhanced source information for Go to Definition
...
class CodeIndexer:
    ...
    def parse_inline_annotations(self, content: str, file_path: Path) -> List[HolographicAnnotation]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if capsule_match:
                ...
                current_annotation.source["style"] = "inline"
                continue
    ...
    def parse_docstring_annotations(self, content: str, file_path: Path) -> List[HolographicAnnotation]:
        ...
        for match in self.docstring_pattern.finditer(content):
            ...
            annotation.source["style"] = "docstring"
    ...
    def parse_decorator_annotations(self, content: str, file_path: Path) -> List[HolographicAnnotation]:
        ...
        for match in self.decorator_pattern.finditer(content):
            ...
            annotation.source["style"] = "decorator"
...
def byte_to_linecol(text: str, byte_idx: int):
    """Convert byte index to line/column position"""
    # treat byte_idx as char offset (we opened as utf-8)
    s = text[:byte_idx]
    line = s.count('\n') + 1
    col = len(s.split('\n')[-1]) + 1
    return line, col

def ollama_generate(prompt: str, model="mistral"):
...
def handle_rpc(request):
    ...
    try:
        ...
        elif method == 'rebuild_faiss' or method == 'rebuild_ann':
            # FAISS rebuild for fast retrieval
            if hasattr(memory_adapter, 'rebuild_faiss'):
                result = memory_adapter.rebuild_faiss()
            else:
                result = {"ok": False, "reason": "faiss-not-supported"}
        elif method == 'explain_capsule':
            # Real Ollama integration for capsule explanation
            capsule_id = params.get('capsule_id')
            model = params.get('model', 'mistral')
            
            # Get capsule details
            capsule_details = memory_adapter.get_capsule_details(capsule_id)
            if not capsule_details:
                result = {"ok": False, "error": "capsule not found"}
            else:
                # Create prompt for Ollama
                prompt = (
                    "Explain this memory capsule succinctly.\n"
                    f"ID: {capsule_id}\n"
                    f"Details: {json.dumps(capsule_details, indent=2)}\n"
                    "Return a clear, human-readable summary of what this capsule represents."
                )
...
def main():
    ...
    # Don't print startup message as it interferes with JSON-RPC
