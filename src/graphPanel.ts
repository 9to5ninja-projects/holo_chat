...
export class HoloGraphPanel {
    public static currentPanel: HoloGraphPanel | undefined;
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, private bridge: PythonBridge) {
        this._panel = panel;
    ...
    private async _handleRecallSlot(data: any) {
        try {
                ...
                for (const ann of annotations) {
                    ...
                    for (const other of annotations) {
                        if (other.capsule_id !== ann.capsule_id) {
                            const sharedSlots = Object.keys(ann.slots || {}).filter(
                                slot => (other.slots || {})[slot] === ann.slots[slot]
                            );
